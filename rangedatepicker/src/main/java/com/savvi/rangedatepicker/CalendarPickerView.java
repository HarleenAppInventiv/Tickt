...
public class CalendarPickerView extends RecyclerView {
    ...
    private Calendar oldSelectDate;
    ...
    private boolean doSelectDate(Date date, MonthCellDescriptor cell) {
        ...
        if (oldSelectDate != null && (oldSelectDate.get(DATE) == newlySelectedCal.get(DATE)
                && oldSelectDate.get(YEAR) == newlySelectedCal.get(YEAR)
                && oldSelectDate.get(MONTH) == newlySelectedCal.get(MONTH))) {
            isEndDatePicked = true;
        } else {
            isEndDatePicked = false;
        }
        oldSelectDate = Calendar.getInstance();
        oldSelectDate.set(DATE, newlySelectedCal.get(DATE));
        oldSelectDate.set(MONTH, newlySelectedCal.get(MONTH));
        oldSelectDate.set(YEAR, newlySelectedCal.get(YEAR));
