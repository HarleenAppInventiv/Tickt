...
internal object BitmapUtils {
     ...
     * contains the requires rectangle, rotate and then crop again a sub rectangle.<br></br>
     * If crop fails due to OOM we scale the cropping image by 0.5 every time it fails until it is small enough.
     */
    fun cropBitmapObjectHandleOOM(bitmap: Bitmap, points: FloatArray, degreesRotated: Int,
                                  fixAspectRatio: Boolean, aspectRatioX: Int, aspectRatioY: Int,
                                  flipHorizontally: Boolean, flipVertically: Boolean): BitmapSampled {
        var scale = 1
        while (true) {
            try {
                val cropBitmap = cropBitmapObjectWithScale(bitmap, points, degreesRotated, fixAspectRatio, aspectRatioX, aspectRatioY,
                        1 / scale.toFloat(), flipHorizontally, flipVertically)
     ...
     */
    private fun cropBitmapObjectWithScale(bitmap: Bitmap, points: FloatArray, degreesRotated: Int,
                                          fixAspectRatio: Boolean, aspectRatioX: Int, aspectRatioY: Int, scale: Float,
                                          flipHorizontally: Boolean, flipVertically: Boolean): Bitmap? {

        // get the rectangle in original image that contains the required cropped area (larger for non rectangular crop)
        val rect = getRectFromPoints(points, bitmap.width, bitmap.height, fixAspectRatio, aspectRatioX, aspectRatioY)
     ...
     */
    fun cropBitmap(context: Context, loadedImageUri: Uri, points: FloatArray,
     ...
     */
    fun getRectFromPoints(points: FloatArray, imageWidth: Int, imageHeight: Int, fixAspectRatio: Boolean, aspectRatioX: Int, aspectRatioY: Int): Rect {
     ...
     *
     * @param mUri the mUri to write the bitmap to, if null
     * @return the mUri where the image was saved in, either the given mUri or new pointing to temp file.
     */
    fun writeTempStateStoreBitmap(context: Context, bitmap: Bitmap, mUri: Uri?): Uri? {
        var uri = mUri
        return try {
            var needSave = true
            if (uri == null) {
    ...
    fun writeBitmapToUri(context: Context, bitmap: Bitmap, uri: Uri, compressFormat: Bitmap.CompressFormat, compressQuality: Int) {
     ...
     */
    fun resizeBitmap(bitmap: Bitmap, reqWidth: Int, reqHeight: Int, options: CropImageView.RequestSizeOptions): Bitmap? {
        try {
     ...
     */
    private fun cropBitmap(context: Context, loadedImageUri: Uri, points: FloatArray, degreesRotated: Int,
                           orgWidth: Int, orgHeight: Int, fixAspectRatio: Boolean, aspectRatioX: Int, aspectRatioY: Int,
                           reqWidth: Int, reqHeight: Int, flipHorizontally: Boolean, flipVertically: Boolean, sampleMulti: Int): BitmapSampled {
     ...
     */
    private fun cropBitmap(context: Context, loadedImageUri: Uri, points: FloatArray,
                           degreesRotated: Int, fixAspectRatio: Boolean, aspectRatioX: Int, aspectRatioY: Int,
                           sampleMulti: Int, rect: Rect, width: Int, height: Int, flipHorizontally: Boolean, flipVertically: Boolean): BitmapSampled {
        var result: Bitmap? = null
        val sampleSize: Int
        try {
            ...
            if (fullBitmap != null) {
                try {
                    ...
                    result = cropBitmapObjectWithScale(fullBitmap, points2, degreesRotated, fixAspectRatio, aspectRatioX, aspectRatioY, 1f,
    ...
    /**
     * Special crop of mBitmap rotated by not stright angle, in this case the original crop mBitmap contains parts
     * beyond the required crop area, this method crops the already cropped and rotated mBitmap to the final
     * rectangle.<br></br>
     * Note: rotating by 0, 90, 180 or 270 degrees doesn't require extra cropping.
     */
    private fun cropForRotatedImage(mBitmap: Bitmap?, points: FloatArray, rect: Rect, degreesRotated: Int,
                                    fixAspectRatio: Boolean, aspectRatioX: Int, aspectRatioY: Int): Bitmap? {
        var bitmap = mBitmap
        if (degreesRotated % 90 != 0) {
     ...
     */
    internal class BitmapSampled(val bitmap: Bitmap?,
                                 val sampleSize: Int)

    /**
     * The result of [.rotateBitmapByExif].
     */
    internal class RotateBitmapResult(val bitmap: Bitmap,
                                      val degrees: Int)
}