...
class CropImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
    ...
    var scaleType: ScaleType?
        ...
        set(scaleType) {
            if (scaleType != mScaleType) {
                ...
                mImageCropperOverlayView!!.resetCropOverlayView()
                requestLayout()
            }
        }
    ...
    var cropShape: CropShape?
        get() = mImageCropperOverlayView!!.cropShape
        set(cropShape) {
            mImageCropperOverlayView!!.cropShape = cropShape
        }
    ...
    var isAutoZoomEnabled: Boolean
        ...
        set(autoZoomEnabled) {
            if (mAutoZoomEnabled != autoZoomEnabled) {
                ...
                mImageCropperOverlayView!!.invalidate()
            }
        }
    ...
    val isFixAspectRatio: Boolean
        get() = mImageCropperOverlayView!!.isFixAspectRatio
    ...
    var guidelines: Guidelines?
        get() = mImageCropperOverlayView!!.guidelines
        set(guidelines) {
            mImageCropperOverlayView!!.guidelines = guidelines
        }
    ...
    val aspectRatio: Pair<Int, Int>
        get() = Pair(mImageCropperOverlayView!!.aspectRatioX, mImageCropperOverlayView.aspectRatioY)
    ...
    var imageResource: Int
        ...
        set(resId) {
            if (resId != 0) {
                mImageCropperOverlayView!!.initialCropWindowRect = null
    ...
    var cropRect: Rect?
        get() {
            if (mBitmap != null) {
                ...
                return BitmapUtils.getRectFromPoints(
                    ...
                    orgWidth,
                    orgHeight,
                    mImageCropperOverlayView!!.isFixAspectRatio,
                    mImageCropperOverlayView.aspectRatioX,
                    mImageCropperOverlayView.aspectRatioY
                )
            } else {
                return null
            }
        ...
        set(rect) {
            mImageCropperOverlayView!!.initialCropWindowRect = rect
        }
    ...
    val cropPoints: FloatArray
        get() {
            val cropWindowRect = mImageCropperOverlayView!!.cropWindowRect
    ...
    init {
        ...
        var options: ImageCropperOptions? = null
        val intent = if (context is Activity) context.intent else null
        if (intent != null) {
            options = intent.getParcelableExtra(ImageCropper.CROP_IMAGE_EXTRA_OPTIONS)
        }
        ...
        mImageCropperOverlayView =
            v.findViewById<View>(R.id.CropOverlayView) as ImageCropperOverlayView
        mImageCropperOverlayView.setCropWindowChangeListener(object :
            ImageCropperOverlayView.CropWindowChangeListener {
        ...
        mImageCropperOverlayView.setInitialAttributeValues(options)
    ...
    fun setMultiTouchEnabled(multiTouchEnabled: Boolean) {
        if (mImageCropperOverlayView!!.setMultiTouchEnabled(multiTouchEnabled)) {
            handleCropWindowChanged(false, false)
            mImageCropperOverlayView.invalidate()
        }
    }
    ...
    fun setMinCropResultSize(minCropResultWidth: Int, minCropResultHeight: Int) {
        mImageCropperOverlayView!!.setMinCropResultSize(minCropResultWidth, minCropResultHeight)
    ...
    fun setMaxCropResultSize(maxCropResultWidth: Int, maxCropResultHeight: Int) {
        mImageCropperOverlayView!!.setMaxCropResultSize(maxCropResultWidth, maxCropResultHeight)
    }
    ...
    fun setFixedAspectRatio(fixAspectRatio: Boolean) {
        mImageCropperOverlayView!!.setFixedAspectRatio(fixAspectRatio)
    }
    ...
    fun setAspectRatio(aspectRatioX: Int, aspectRatioY: Int) {
        mImageCropperOverlayView!!.aspectRatioX = aspectRatioX
        mImageCropperOverlayView.aspectRatioY = aspectRatioY
    ...
    fun clearAspectRatio() {
        mImageCropperOverlayView!!.aspectRatioX = 1
        mImageCropperOverlayView.aspectRatioY = 1
    ...
    fun setSnapRadius(snapRadius: Float) {
        if (snapRadius >= 0) {
            mImageCropperOverlayView!!.setSnapRadius(snapRadius)
        }
    }
    ...
    fun resetCropRect() {
        ...
        mImageCropperOverlayView!!.resetCropWindowRect()
    }

    /**
     * Gets the cropped image based on the current crop window.<br></br>
     *
     * @param width the width to resize the cropped image to (see options)
     * @param height the height to resize the cropped image to (see options)
    ...
    /**
     * Rotates image by the specified number of deg clockwise.<br></br>
     * Negative values represent counter-clockwise rotations.
     *
     * @param deg Integer specifying the number of deg to rotate.
     */
    fun rotateImage(deg: Int) {
        var degrees = deg
        if (mBitmap != null) {
            // Force deg to be a non-zero value between 0 and 360 (inclusive)
            if (degrees < 0) {
            ...
            val flipAxes =
                !mImageCropperOverlayView!!.isFixAspectRatio && (degrees > 45 && degrees < 135 || degrees > 215 && degrees < 305)
            BitmapUtils.RECT.set(mImageCropperOverlayView.cropWindowRect)
            var halfWidth =
            ...
            // This is valid because deg is not negative.
            ...
            mImageCropperOverlayView.resetCropOverlayView()
            mImageCropperOverlayView.cropWindowRect = BitmapUtils.RECT
            applyImageMatrix(width.toFloat(), height.toFloat(), true, false)
            handleCropWindowChanged(false, false)

            // make sure the crop window rectangle is within the cropping image bounds after all the changes
            mImageCropperOverlayView.fixCurrentCropWindowRect()
        }
    }
    ...
    internal fun onImageCroppingAsyncComplete(result: BitmapCroppingWorkerTask.Result) {
        ...
        if (listener != null ) {
            val cropResult = CropResult(
                mBitmap!!, imageUri!!, result.bitmap!!, result.uri!!, result.error,
                cropPoints, cropRect!!, rotatedDegrees, result.sampleSize
            )
    ...
    private fun setBitmap(
        ...
        if (mBitmap == null || mBitmap != bitmap) {
            ...
            if (mImageCropperOverlayView != null) {
                mImageCropperOverlayView.resetCropOverlayView()
                setCropOverlayVisibility()
            }
        }
    ...
    /**
     * Gets the cropped image based on the current crop window.<br></br>
     * If (width,height) is given AND image is loaded from URI cropping will try to use sample size to fit in
    ...
    public override fun onSaveInstanceState(): Parcelable? {
        ...
        bundle.putParcelable("INITIAL_CROP_RECT", mImageCropperOverlayView!!.initialCropWindowRect)
        ...
        bundle.putString("CROP_SHAPE", mImageCropperOverlayView.cropShape!!.name)
        ...
        if (state is Bundle) {
            ...
            if (mBitmapLoadingWorkerTask == null && imageUri == null && mBitmap == null && mImageResource == 0) {
                ...
                if (initialCropRect != null && (initialCropRect.width() > 0 || initialCropRect.height() > 0)) {
                    mImageCropperOverlayView!!.initialCropWindowRect = initialCropRect
                }
                ...
                mImageCropperOverlayView!!.cropShape =
                    CropShape.valueOf(state.getString("CROP_SHAPE")!!)
        ...
        if (mLayoutWidth > 0 && mLayoutHeight > 0) {
            ...
            if (mBitmap != null) {
                ...
                if (mRestoreCropWindowRect != null) {
                    ...
                    mImageCropperOverlayView!!.cropWindowRect = mRestoreCropWindowRect as RectF
                    handleCropWindowChanged(false, false)
                    mImageCropperOverlayView.fixCurrentCropWindowRect()
                    mRestoreCropWindowRect = null
    ...
    private fun handleCropWindowChanged(inProgress: Boolean, animate: Boolean) {
        ...
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            val cropRect = mImageCropperOverlayView!!.cropWindowRect
            if (inProgress) {
                ...
                if (newZoom > 0 && newZoom != mZoom) {
                    if (animate) {
                        if (mAnimation == null) {
                            ...
                            mAnimation = ImageCropperAnimation(mImageView, mImageCropperOverlayView)
                        }
    ...
    private fun applyImageMatrix(width: Float, height: Float, center: Boolean, animate: Boolean) {
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            mImageCropperOverlayView.cropWindowRect = cropRect
            mapImagePointsByImageMatrix()
            mImageCropperOverlayView.invalidate()
    ...
    private fun setCropOverlayVisibility() {
        if (mImageCropperOverlayView != null) {
            mImageCropperOverlayView.visibility =
    ...
    private fun setProgressBarVisibility() {
        val visible =
            mShowProgressBar && (mBitmap == null && mBitmapLoadingWorkerTask != null || mBitmapCroppingWorkerTask != null)
    ...
    private fun updateImageBounds(clear: Boolean) {
        if (mBitmap != null && !clear) {
            ...
            val scaleFactorWidth =
                mBitmap!!.width * mLoadedSampleSize / BitmapUtils.getRectWidth(mImagePoints)
            val scaleFactorHeight =
                mBitmap!!.height * mLoadedSampleSize / BitmapUtils.getRectHeight(mImagePoints)
            mImageCropperOverlayView!!.setCropWindowLimits(
                width.toFloat(),
                height.toFloat(),
                scaleFactorWidth,
                scaleFactorHeight
        ...
        mImageCropperOverlayView!!.setBounds(if (clear) null else mImagePoints, width, height)
    }
    //endregion
