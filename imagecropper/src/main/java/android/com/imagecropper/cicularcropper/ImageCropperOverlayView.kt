 ...
 */
class ImageCropperOverlayView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
     ...
     */
    private val mImageCropperCropWindowHandler = ImageCropperCropWindowHandler()
     ...
     */
    private var mMoveHandler: ImageCropperWindowMoveHandler? = null
    ...
    var cropWindowRect: RectF
        get() = mImageCropperCropWindowHandler.rect
        set(rect) {
            mImageCropperCropWindowHandler.rect = rect
        }
    ...
    fun setBounds(boundsPoints: FloatArray?, viewWidth: Int, viewHeight: Int) {
        if (boundsPoints == null || !Arrays.equals(mBoundsPoints, boundsPoints)) {
            ...
            val cropRect = mImageCropperCropWindowHandler.rect
     ...
     */
    fun setInitialAttributeValues(options: ImageCropperOptions) {
    ...
    private fun initCropWindow() {
        ...
        if (initialCropWindowRect!!.width() > 0 && initialCropWindowRect!!.height() > 0) {
            ...
            rect.left = leftLimit + initialCropWindowRect!!.left / mImageCropperCropWindowHandler.scaleFactorWidth
            rect.top = topLimit + initialCropWindowRect!!.top / mImageCropperCropWindowHandler.scaleFactorHeight
            rect.right = rect.left + initialCropWindowRect!!.width() / mImageCropperCropWindowHandler.scaleFactorWidth
            rect.bottom = rect.top + initialCropWindowRect!!.height() / mImageCropperCropWindowHandler.scaleFactorHeight
            ...
            if (bitmapAspectRatio > mTargetAspectRatio) {
                ...
                val cropWidth = Math.max(mImageCropperCropWindowHandler.minCropWidth, rect.height() * mTargetAspectRatio)
                ...
                // Limits the aspect ratio to no less than 40 wide or 40 tall
                val cropHeight = Math.max(mImageCropperCropWindowHandler.minCropHeight, rect.width() / mTargetAspectRatio)
    ...
    private fun fixCropWindowRectByRules(rect: RectF) {
        if (rect.width() < mImageCropperCropWindowHandler.minCropWidth) {
            val adj = (mImageCropperCropWindowHandler.minCropWidth - rect.width()) / 2
            rect.left -= adj
            rect.right += adj
        }
        if (rect.height() < mImageCropperCropWindowHandler.minCropHeight) {
            val adj = (mImageCropperCropWindowHandler.minCropHeight - rect.height()) / 2
            rect.top -= adj
            rect.bottom += adj
        }
        if (rect.width() > mImageCropperCropWindowHandler.maxCropWidth) {
            val adj = (rect.width() - mImageCropperCropWindowHandler.maxCropWidth) / 2
            rect.left += adj
            rect.right -= adj
        }
        if (rect.height() > mImageCropperCropWindowHandler.maxCropHeight) {
            val adj = (rect.height() - mImageCropperCropWindowHandler.maxCropHeight) / 2
            rect.top += adj
            rect.bottom -= adj
        }
    ...
    private fun drawBackground(canvas: Canvas) {
        ...
        val rect = mImageCropperCropWindowHandler.rect
        ...
        if (cropShape == CropImageView.CropShape.RECTANGLE) {
            ...
            mPath.reset()
            if (Build.VERSION.SDK_INT >= 11 && Build.VERSION.SDK_INT <= 17 && cropShape == CropImageView.CropShape.OVAL) {
    ...
    private fun onActionDown(x: Float, y: Float) {
        mMoveHandler = mImageCropperCropWindowHandler.getMoveHandler(x, y, mTouchRadius, cropShape!!)
    ...
    private fun onActionMove(x: Float, y: Float) {
        if (mMoveHandler != null) {
            ...
            mMoveHandler!!.move(rect, x, y, mCalcBounds, mViewWidth, mViewHeight, snapRadius, isFixAspectRatio, mTargetAspectRatio)
            mImageCropperCropWindowHandler.rect = rect
    ...
    private inner class ScaleListener : ScaleGestureDetector.SimpleOnScaleGestureListener() {
        ...
        override fun onScale(detector: ScaleGestureDetector): Boolean {
            ...
            if (newLeft < newRight &&
                    newTop <= newBottom &&
                    newLeft >= 0 &&
                    newRight <= mImageCropperCropWindowHandler.maxCropWidth &&
                    newTop >= 0 &&
                    newBottom <= mImageCropperCropWindowHandler.maxCropHeight) {
