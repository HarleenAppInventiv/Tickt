...
class UploadImagesAdapter(var photos: ArrayList<Photos>, val listener: View.OnClickListener, val adapterListener:ClickListener) :
        ...
        when (holder) {
            is ViewHolderAnswer -> {
                ...
                holder.binding.ivCancelImage.setOnClickListener {
                   ...
                   if(photos.size==1 && photos[photos.size - 1].mediaType == 0)
                   {
                       photos.removeAt(0)
                       notifyDataSetChanged()
                   }
                    ...
                    adapterListener.onCancelCLick(position)
