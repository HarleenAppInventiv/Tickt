...
// (abed): remove [getPathFromUri], [getVideoExtension], and [copy]
...
fun returnVideoFileName(): File? {
    val imageFileName: File
    val ImageName = "Video_" + Calendar.getInstance().getTimeInMillis().toString() + ".mp4"
    val Imagefolder: String = createFolder("Ticket/Video")
    imageFileName = if (Imagefolder != "") {
        File(Imagefolder, ImageName)
    } else {
        return null
    }
...
fun createFolder(folderPath: String): String {
    var folderName = ""
    folderName = Environment.getExternalStorageDirectory().toString() + "/" + folderPath
    // Log.d("", "folderName : " + folderName);
    val fileName = File(folderName)
    if (!fileName.exists()) {
        fileName.mkdirs()
    }
    return folderName
